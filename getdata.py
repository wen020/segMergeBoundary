...
train_boundarys_output_dir = "./data/train/boundarys"
val_imgs_output_dir = "./data/val/images"
val_masks_output_dir = "./data/val/masks"
val_boundarys_output_dir = "./data/val/boundarys"
test_imgs_output_dir = "./data/test/images"
test_masks_output_dir = "./data/test/masks"
test_boundarys_output_dir = "./data/test/boundarys"
if not os.path.exists(train_imgs_output_dir):
    os.makedirs(train_imgs_output_dir)
if not os.path.exists(train_boundarys_output_dir):
    os.makedirs(train_boundarys_output_dir)
...
if not os.path.exists(val_boundarys_output_dir):
    os.makedirs(val_boundarys_output_dir)
...
if not os.path.exists(test_boundarys_output_dir):
    os.makedirs(test_boundarys_output_dir)
...
boundary_paths = sorted(glob.glob(os.path.join(boundarys_dir, "*.tif")))
print(img_paths)
print(mask_paths)
...
def get_img_mask_padded(image, mask, boundary, patch_size, mode):
    img, mask, boundary = np.array(image), np.array(mask), np.array(boundary)
    ...
    pad_boundary = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right',
                            border_mode=0, value=[0, 0, 0])(image=boundary)
    img_pad, mask_pad, boundary_pad = pad_img['image'], pad_mask['image'], pad_boundary['image']
    img_pad = cv2.cvtColor(np.array(img_pad), cv2.COLOR_RGB2BGR)
    mask_pad = cv2.cvtColor(np.array(mask_pad), cv2.COLOR_RGB2BGR)
    boundary_pad = cv2.cvtColor(np.array(boundary_pad), cv2.COLOR_RGB2BGR)
    return img_pad, mask_pad, boundary_pad
...
def rgb_to_2D_label(_mask, _boundary):
    print(_mask.size)
    _mask = _mask.transpose(2, 0, 1)
    mask_seg = np.zeros(_mask.shape[1:], dtype=np.uint8)
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == ImSurf, axis=-1)] = 0
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == Building, axis=-1)] = 1
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == LowVeg, axis=-1)] = 2
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == Tree, axis=-1)] = 3
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == Car, axis=-1)] = 4
    mask_seg[np.all(_mask.transpose([1, 2, 0]) == Clutter, axis=-1)] = 5

    _boundary = _boundary.transpose(2, 0, 1)
    boundary_seg = np.zeros(_boundary.shape[1:], dtype=np.uint8)
    boundary_seg[np.all(_boundary.transpose([1, 2, 0]) == Boundary, axis=-1)] = 1
    return mask_seg, boundary_seg


def image_augment(image, mask, boundary, patch_size, mode='train', val_scale=1.0):
    image_list = []
    mask_list = []
    boundary_list = []
    ...
    if mode == 'train':
        ...
        image_h_vlip, mask_h_vlip, boundary_h_vlip = h_vlip(image.copy()), h_vlip(mask.copy()), h_vlip(boundary.copy())
        image_v_vlip, mask_v_vlip, boundary_v_vlip = v_vlip(image.copy()), v_vlip(mask.copy()), v_vlip(boundary.copy())
        ...
        boundary_list_train = [boundary, boundary_h_vlip, boundary_v_vlip]
        # image_list_train = [image]
        # mask_list_train = [mask]
        for i in range(len(image_list_train)):
            image_tmp, mask_tmp, boundary_tmp = get_img_mask_padded(image_list_train[i], mask_list_train[i], boundary_list_train[i], patch_size, mode)
            mask_tmp, boundary_tmp = rgb_to_2D_label(mask_tmp.copy(), boundary_tmp.copy())
            image_list.append(image_tmp)
            mask_list.append(mask_tmp)
            boundary_list.append(boundary_tmp)
    else:
        rescale = Resize(size=(int(image_width * val_scale), int(image_height * val_scale)))
        image, mask, boundary = rescale(image.copy()), rescale(mask.copy()), rescale(boundary.copy())
        image, mask, boundary = get_img_mask_padded(image.copy(), mask.copy(), boundary.copy(), patch_size, mode)
        mask, boundary = rgb_to_2D_label(mask.copy(), boundary.copy())
        image_list.append(image)
        mask_list.append(mask)
        boundary_list.append(boundary)
    return image_list, mask_list, boundary_list


def randomsizedcrop(image, mask, boundary):
    # assert image.shape[:2] == mask.shape
    h, w = image.shape[0], image.shape[1]
    crop = albu.RandomSizedCrop(min_max_height=(int(3*h//8), int(h//2)), width=h, height=w)(image=image.copy(), mask=mask.copy(), boundary=boundary.copy())
    img_crop, mask_crop, boundary_crop = crop['image'], crop['mask'], crop['boundary']
    return img_crop, mask_crop, boundary_crop


def car_aug(image, mask, boundary):
    assert image.shape[:2] == mask.shape
    v_flip = albu.VerticalFlip(p=1.0)(image=image.copy(), mask=mask.copy(), boundary=boundary.copy())
    h_flip = albu.HorizontalFlip(p=1.0)(image=image.copy(), mask=mask.copy(), boundary=boundary.copy())
    rotate_90 = albu.RandomRotate90(p=1.0)(image=image.copy(), mask=mask.copy(), boundary=boundary.copy())
    # blur = albu.GaussianBlur(p=1.0)(image=image.copy())
    image_vflip, mask_vflip, boundary_vflip = v_flip['image'], v_flip['mask'], v_flip['boundary']
    image_hflip, mask_hflip, boundary_hflip = h_flip['image'], h_flip['mask'], h_flip['boundary']
    image_rotate, mask_rotate, boundary_rotate = rotate_90['image'], rotate_90['mask'], rotate_90['boundary']
    ...
    boundary_list = [boundary, boundary_vflip, boundary_hflip, boundary_rotate]

    return image_list, mask_list, boundary_list

def vaihingen_format(inp):
    (img_path, mask_path, boundary_path, imgs_output_dir, masks_output_dir, boundarys_output_dir, eroded, gt, mode, val_scale, split_size, stride) = inp
    img_filename = os.path.splitext(os.path.basename(img_path))[0]
    mask_filename = os.path.splitext(os.path.basename(mask_path))[0]
    boundary_filename = os.path.splitext(os.path.basename(boundary_path))[0]
    if eroded:
    ...
    image_pad, mask_pad, boundary_pad = get_img_mask_padded(img.copy(), mask.copy(), boundary.copy(), 16, "train")
    mask_tmp, boundary_tmp = rgb_to_2D_label(mask_pad.copy(),boundary_pad.copy())
    ...
    image_list, mask_list, boundary_list = image_augment(image=img.copy(), mask=mask.copy(), boundary=boundary.copy(), patch_size=split_size,
    ...
    for m in range(len(image_list)):
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                boundary_tile = boundary[y:y + split_size, x:x + split_size]
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    image_crop, mask_crop, boundary_crop = randomsizedcrop(img_tile, mask_tile, boundary_tile)
                    ...
                    if cf[4] > 0.1 and mode == 'train':
                        car_imgs, car_masks, car_boundarys = car_aug(image_crop, mask_crop, boundary_crop)
                        for i in range(len(car_imgs)):
                            ...
                            out_boundary_path = os.path.join(boundarys_output_dir,
                                                         "{}_{}_{}_{}.png".format(boundary_filename, m, k, i))
                            cv2.imwrite(out_boundary_path, car_boundarys[i])
                    else:
                        ...
                        out_boundary_path = os.path.join(boundarys_output_dir, "{}_{}_{}.png".format(boundary_filename, m, k))
                        cv2.imwrite(out_boundary_path, boundary_tile)

                k += 1



for img_path, mask_path, boundary_path in zip(img_paths, mask_paths, boundary_paths):
    img_filename = os.path.splitext(os.path.basename(img_path))[0]
    print(img_filename)
    mask_filename = os.path.splitext(os.path.basename(mask_path))[0]
    boundary_path = os.path.join(boundarys_dir, img_filename + '_noBoundary.tif')
    print(img_path, mask_path, boundary_path)
    # mask_path = mask_path[:-4] + '_noBoundary.tif'
    ...
    if img_filename in train_data:
        inp = (img_path, mask_path, boundary_path, train_imgs_output_dir, train_masks_output_dir, train_boundarys_output_dir, False, False, "train", 1.0, 1024, 512)
        vaihingen_format(inp)
    else:
        inp = (img_path, mask_path, boundary_path, val_imgs_output_dir, val_masks_output_dir, val_boundarys_output_dir, False, False, "val", 1.0, 1024, 1024)
        vaihingen_format(inp)
